...
#pragma omp for [клауза [ клауза]]
    цикл for
```
* **private (список)**   Переменная, доступная только одному потоку в группе, выполняющие параллельную область.   
* **firstprivate (список)**  Переменные получают свои значения из последовательной части программы
* **lastprivate (список)** Переменные сохраняют свои значения при выходе из параллельных потоков при условии их последовательного выполнения.
