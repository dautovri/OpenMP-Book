...
Перменная окружения позволяет отобразить версию номер версии OpenMP и значения переменных окружения, которые относятся к OpenMP. Данная информация выводится один раз до вызова пользовательских функций. 
```
